...
void Axis::update(const glm::mat4 &rotation)
{
  ...
  this->_current_right = glm::normalize(glm::cross(this->_current_direction, this->_initial_up));
  this->_current_up = glm::normalize(glm::cross(this->_current_direction, this->_current_right));
}
